...
int RemHated( struct char_data *ch, struct char_data *pud)
{
  ...
  if (!ch->hates.clist) {
    CLEAR_BIT(ch->hatefield, HATE_CHAR);
  }
  if (!ch->hatefield)
    CLEAR_BIT(ch->specials.act, ACT_HATEFUL);
...
int RemFeared( struct char_data *ch, struct char_data *pud)
{
  ...
  if (!ch->fears.clist)
    CLEAR_BIT(ch->fearfield, FEAR_CHAR);
  if (!ch->fearfield)
    CLEAR_BIT(ch->specials.act, ACT_AFRAID);
  return( (pud) ? TRUE : FALSE);
}
