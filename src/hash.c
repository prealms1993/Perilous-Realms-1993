...
if (DEBUG>10) vlog(LOG_DEBUG,"init_hash_table");
  ht->rec_size = rec_size;
  ht->table_size = table_size;
  ht->buckets = (void*)calloc(sizeof(struct hash_link**), table_size);
  ht->keylist = (void*)malloc(sizeof(key_type)*(ht->klistsize=128));
  ht->klistlen = 0;
}
...
if (DEBUG>10) vlog(LOG_DEBUG,"destroy_hash_table");
...
if (DEBUG>1000) log("hash_iterate");
    key = ht->keylist[i];
    temp = hash_find(ht, key);
    (*func)(key, temp, cdata);
