...
void TrapDamage(struct char_data *v, int damtype, int amnt, struct obj_data *t)
{
  ...
  if (IS_AFFECTED(v, AFF_SANCTUARY))
   ...
   amnt=MIN(GET_HIT(v) - 1, MAX(amnt,0)); /* temp fix, to prevent dying */
...
/*   if (GET_POS(v) == POSITION_DEAD) {
      if (!IS_NPC(v)) {
	     ...
	     vlog(LOG_DEATH,"%s killed by a trap at %s",
	       		GET_NAME(v),
...
void TrapTeleport(struct char_data *v) 
{
    ...
    if (saves_spell(v,SAVING_SPELL)) {
     ...
     do {
      ...
      || IS_SET(real_roomp(to_room)->room_flags, NO_GOTO)
      || IS_SET(real_roomp(to_room)->room_flags, LORD_ONLY));
